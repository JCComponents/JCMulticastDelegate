...
@interface JCMulticastDelegateNode : NSObject {
@private
...
@interface JCMulticastDelegateEnumerator ()
{
	NSUInteger numNodes;
...
@implementation JCMulticastDelegate
...
- (void)addDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
	...
	JCMulticastDelegateNode *node =
	    [[JCMulticastDelegateNode alloc] initWithDelegate:delegate delegateQueue:delegateQueue];
...
- (void)removeDelegate:(id)delegate delegateQueue:(dispatch_queue_t)delegateQueue
{
	...
	{
		JCMulticastDelegateNode *node = [delegateNodes objectAtIndex:(i-1)];
		...
		if (delegate == nodeDelegate)
		{
			...
			{
				// Recall that this node may be retained by a JCMulticastDelegateEnumerator.
...
{
	for (JCMulticastDelegateNode *node in delegateNodes)
	{
		node.delegate = nil;
		#if __has_feature(objc_arc_weak) && !TARGET_OS_IPHONE
		node.unsafeDelegate = nil;
...
- (JCMulticastDelegateEnumerator *)delegateEnumerator
{
	return [[JCMulticastDelegateEnumerator alloc] initFromDelegateNodes:delegateNodes];
}
...
@implementation JCMulticastDelegateNode
...
@implementation JCMulticastDelegateEnumerator
...
- (BOOL)getNextDelegate:(id *)delPtr delegateQueue:(dispatch_queue_t *)dqPtr
{
	...
	{
		JCMulticastDelegateNode *node = [delegateNodes objectAtIndex:currentNodeIndex];
		currentNodeIndex++;
